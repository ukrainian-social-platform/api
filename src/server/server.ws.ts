...
import { Server, GrpcMethodStreamingType } from '@nestjs/microservices';
...
type DefaultValue<Value, Resolve> = Value extends () => any
	? Resolve extends true
		? ReturnType<Value>
		: Value
	: Value;

type DynamicProp<T> = T | (() => T);

interface WebSocketServerOptions {
	port?: DynamicProp<number>;
	protoLoader?: DynamicProp<string>;
}

const transportId = Symbol('WebSocket Server');

interface ProtoLoaderPackage {
	loadSync(file: string): any;
}

@Name(ServerWebSocket.name)
export class ServerWebSocket extends Server implements CustomTransportStrategy {
	public readonly transportId = transportId;

	private readonly url: string;
	private grpcProtoLoaderPackage: ProtoLoaderPackage;
	private wss: WebSocketServer;

	constructor(private readonly options: WebSocketServerOptions) {
		super();
		this.url = this.getOptionsPropExt('url') || GRPC_DEFAULT_URL;

		const protoLoader =
			this.getOptionsPropExt('protoLoader') || GRPC_DEFAULT_PROTO_LOADER;

		this.grpcProtoLoaderPackage = this.loadPackage(
			protoLoader,
			ServerWebSocket.name,
			() =>
	...
	public async listen(
		...
		this.wss = new WebSocketServer({
			port: this.getOptionsPropExt('port'),
		});
	...
	public async bindEvents() {
		...
		const packageOption = this.getOptionsPropExt('package');
	...
	public async createClient(): Promise<any> {
		...
		if (this.options && this.options.maxMetadataSize) {
			channelOptions['grpc.max_metadata_size'] =
				this.options.maxMetadataSize;
		}
		const server = new grpcPackage.Server(channelOptions);
		const credentials = this.getOptionsPropExt('credentials');
	...
	public loadProto(): any {
		try {
			const file = this.getOptionsPropExt('protoPath');
			const loader = this.getOptionsPropExt('loader');

			const packageDefinition = grpcProtoLoaderPackage.loadSync(
				file,
				loader,
			);
			const packageObject =
	...
	getOptionsPropExt<
		T extends keyof WebSocketServerOptions,
		R extends boolean = true,
	>(
		option: T,
		defaultValue?: DefaultValue<WebSocketServerOptions[T], R>,
		resolveFunction?: R,
	): DefaultValue<WebSocketServerOptions[T], R> {
		const { options } = this;
		if (!options || !(option in options)) return defaultValue;
		const resolvedOption = options[option];
		return (resolveFunction === undefined || resolveFunction) &&
			typeof resolvedOption === 'function'
			? resolvedOption()
			: resolvedOption;
	}
}
